...
import { ArrowLeft, Phone, MapPin, CreditCard, ShoppingBag, Calendar, Receipt, CheckCircle2 } from 'lucide-react';
...
export default function CustomerLedger({ customerId }: CustomerLedgerProps) {
  const { customers, getCustomerLedgerData, setSelectedCustomerId, setScaffoldAction } = useStockFlow();
  ...
  return (
    <div className="space-y-4">
      {/* Top Header matching Screens 10 & 12 */}
      <div className="flex items-center justify-between pb-2 border-b border-slate-200/80">
        ...
        >
          <span className="text-[10px] block font-medium opacity-80 uppercase tracking-wider">Outstanding</span>
          <span>{summary.outstandingBalance.toLocaleString()} ETB</span>
        </div>
      </div>
      ...
      {/* 3 Metric Summary Cards matching Screen 10 / 12 */}
      <div className="grid grid-cols-3 gap-2.5">
        {/* Total Sales */}
        <div className="bg-[#ECFDF5] border border-emerald-100 rounded-2xl p-3 text-center shadow-2xs">
          <p className="text-[10px] font-semibold text-emerald-800 uppercase tracking-wider">Total Sales</p>
        ...
        <div className="bg-[#F0FDF4] border border-slate-200/70 rounded-2xl p-3 text-center shadow-2xs">
          <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-wider">Total Paid</p>
          <p className="text-base sm:text-lg font-extrabold text-slate-900 mt-0.5">
        ...
        <div className="bg-[#FFF1F2] border border-rose-100 rounded-2xl p-3 text-center shadow-2xs">
          <p className="text-[10px] font-semibold text-rose-800 uppercase tracking-wider">Outstanding</p>
          <p className="text-base sm:text-lg font-extrabold text-rose-700 mt-0.5">
      ...
      {/* Primary Action Buttons matching Screen 10 */}
      <div className="grid grid-cols-2 gap-2.5">
        <button
          type="button"
          onClick={() => setScaffoldAction('payment')}
        ...
        <button
          type="button"
          onClick={() => setScaffoldAction('sale')}
      ...
      {/* Segmented Control Tabs matching Screen 10 */}
